...
int ListaDoble::insertar(DATOS datos_insertar, void funcionInsertar(DATOS d), bool funcionOrdenamiento(DATOS nuevo, DATOS insertado, char tipo_orden))
{
	ENLACE nuevo;
	ENLACE head, tmp;
	unsigned int r = 0;
...
int ListaDoble::mostrar(void funcionMostrarLista(DATOS d, int r))
{
...
int ListaDoble::insertarMultiple(void funcionInsertar(DATOS d),bool funcionOrdenamiento(DATOS nuevo, DATOS insertado, char tipo_orden), int numero, ...)
{
    int i = 0;
    int r = 1;
    va_list lt;
...
{
    if(this->Struct_Index == NULL || i >= this->numElementosStruct_Index )
        return NULL;
    else 
...
bool ListaDoble::eliminar(DATOS d, bool funcionEliminarLista(DATOS d, DATOS a_eliminar))
{
    estructuraListaDoble *p = this->Struct;
    int r =0;
    if (this->Struct == NULL)
...
DATOS **ListaDoble::buscarAll(DATOS d, bool funcionBuscarAllLista(DATOS d, DATOS a_buscar))
{
    estructuraListaDoble *p = this->Struct;
    DATOS **r=NULL;
    int cont = 0;
    ...
    if (this->Struct == NULL)
        ...
        while (p != NULL) {
            /* CODIGO AQUI ---> */
            if(funcionBuscarAllLista(p->pdatos, d)){
                //r = (DATOS*)p->pdatos;
                cont++;
            }
            /* <--------------- */
        ...
        p = this->Struct;
        r = (DATOS**)calloc(cont, sizeof (DATOS));
        cont = 0;
        while(p != NULL)
        {
            if(funcionBuscarAllLista(p->pdatos, d)){
                r[cont++] = (DATOS*)p->pdatos;
            }
            /* <--------------- */
            p = p->siguiente;
        }  
...
int ListaDoble::eliminarAll(DATOS d, bool funcionEliminarAllLista(DATOS d, DATOS a_eliminar))
{
    estructuraListaDoble *p = this->Struct;
    int r =0;
    ...
    if (this->Struct == NULL)
        ...
        while (p != NULL) {
            /* CODIGO AQUI ---> */
            if(funcionEliminarAllLista(p->pdatos, d)){
                if(this->numElementos > 0) {   
                    if(p->anterior == NULL){
                        p->siguiente->anterior = NULL;
                        this->Struct = p->siguiente;
